...
List<Class> buildOperationReqClasses(SourceNode docSource) =>
    docSource.document.definitions
        .whereType<OperationDefinitionNode>()
        .map(_buildOperationReqClass)
        .toList();

Class _buildOperationReqClass(OperationDefinitionNode node) => builtClass(
  name: node.name!.value,
  getters: [
    Method(
      (b) =>
          b
            ..returns = refer("${builtClassName(node.name!.value)}Vars", "#var")
