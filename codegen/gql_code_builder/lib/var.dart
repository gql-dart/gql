...
Library buildVarLibrary(
  ...
  final fragmentVarClasses =
      docSource.document.definitions.whereType<FragmentDefinitionNode>().map((
        frag,
      ) {
...
Map<String, Expression> _varClassValueInitializers(
  ...
  for (final node in op.variableDefinitions.where(
    (element) => !element.type.isNonNull,
  ))
